...
module "ec2_autoscale" {
  ...
  #Attribute-based instance type selection (CPU/Memory instead of instance types)
  ...
  mixed_instances_overrides = [
    {
      instance_requirements = {
        ...
        # Require 4-8 GiB of memory
        memory_mib = {
          min = 4096
          max = 8192
        }
        # Only consider current-generation instance families
        instance_generations = ["current"]
        # Only let AWS pick from these instance families (an include-list) instead of
        # excluding families with `excluded_instance_types`. Wildcards are supported,
        # e.g. "m5.*" matches every size in the m5 family. AWS does not allow setting
        # both `allowed_instance_types` and `excluded_instance_types` at the same time.
        allowed_instance_types = ["m5.*", "m5a.*", "m6i.*", "c5.*"]
        # excluded_instance_types = ["m5.*", "m5a.*", "m6i.*", "c5.*"]
      }
    }
  ]
